...
### 2. 예외 처리 및 Fallback 로직 설계 (핵심 기여)
- 음성 인식 환경에서 발생할 수 있는 '무응답 예외 상황'을 방지하기 위해 **10초 타이머 기반의 Fallback 메커니즘을 구현**했습니다.
- 10초간 응답이 없을 경우, 시스템이 완전히 멈추는 대신 **사용자에게 시각적 팝업을 통해 수동 선택을 유도**하고, 진행 중이던 TTS와 STT를 안전하게 중단(Clear)시켜 자원 낭비와 버그를 막았습니다.

### 3. 사용자 식별을 위한 고유번호 생성 및 임시 결과창 팝업 로직 기여
- **고유번호 생성 로직:** 의료 데이터의 특성상 철저한 익명성과 중복 방지가 중요함을 인지하고, 유저 세션을 안전하게 관리할 수 있는 **고유번호 생성 가이드라인 및 로직 아이디어를 제공**했습니다.
- **임시 결과창 팝업:** 설문이 종료된 직후 유저가 자신의 진단 결과를 직관적으로 확인할 수 있도록, **데이터 전송 완료 시점에 맞추어 최종 점수를 시각화하는 임시 결과창 팝업 로직 아이디어를 제안하고 프론트엔드 연동을 지원**했습니다.
- 이를 통해 데이터가 백엔드(FastAPI)와 프론트엔드(SvelteKit) 사이에서 유실 없이 매끄럽게 흐르도록 데이터 흐름 검증을 철저히 수행했습니다.
